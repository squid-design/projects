...
module Jekyll
  ...
  class Picture < Liquid::Tag
    ...
    def render(context)
      ...
      markup = /^(?:(?<preset>[^\s.:\/]+)\s+)?(?<image_src>[^\s]+\.[a-zA-Z0-9]{3,4})\s*(?<source_src>(?:(source_[^\s.:\/]+:\s+[^\s]+\.[a-zA-Z0-9]{3,4})\s*)+)?(?<caption>"([^"]*)")?(?<html_attr>[\s\S]+)?$/.match(render_markup)
      ...
      if markup['caption']
          caption = markup['caption'].chomp('"').reverse.chomp('"').reverse
      else
          caption = ''
      end
      ...
      if settings['markup'] == 'picture'
        ...
        # Note: Thumbnail should be applied to all post-images, as far as I can tell.
        # if !caption.empty?
            # picture_class = "class='thumbnail'"
        # else
            # picture_class = "class=''"
        # end
        picture_class = "class='thumbnail'"
